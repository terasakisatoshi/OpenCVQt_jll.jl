...
if VERSION < v"1.3.0-rc4"
    ...
    error("Unable to import OpenCVQt_jll on Julia versions older than 1.3!")
end
    ...
    function dev_jll()
        ...
        mktempdir() do temp_env
            Pkg.activate(temp_env) do
                Pkg.develop("OpenCVQt_jll")
            end
        end
        # Create the override directory
        override_dir = joinpath(Pkg.devdir(), "OpenCVQt_jll", "override")
        # Copy the current artifact contents into that directory
        if !isdir(override_dir)
            cp(artifact"OpenCVQt", override_dir)
        end
        # Force recompilation of that package, just in case it wasn't dev'ed before
        touch(joinpath(Pkg.devdir(), "OpenCVQt_jll", "src", "OpenCVQt_jll.jl"))
        @info("OpenCVQt_ll dev'ed out to /home/terasaki/.julia/dev/OpenCVQt_jll with pre-populated override directory")
    end
end
...
artifacts = Pkg.Artifacts.load_artifacts_toml(artifacts_toml; pkg_uuid=UUID("2e4a2372-4a7c-5bb9-ae2d-b1938f154ce2"))
platforms = [Pkg.Artifacts.unpack_platform(e, "OpenCVQt", artifacts_toml) for e in artifacts["OpenCVQt"]]
...
if best_platform === nothing
    @debug("Unable to load OpenCVQt; unsupported platform $(triplet(platform_key_abi()))")
else
